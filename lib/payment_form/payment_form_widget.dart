            ...
            Align(
              ...
              child: Padding(
                ...
                child: FFButtonWidget(
                  onPressed: () async {
                    _model.apiResultrci = await PostOrderCall.call(
                      contactName: _model.yourNameTextController.text,
                      contactLastname: _model.cityTextController1.text,
                      contactEmail: _model.cityTextController2.text,
                      productsJson: functions.listToJson(widget.cart!.toList()),
                    );
                    if ((_model.apiResultrci?.succeeded ?? true)) {
                      await OrdersTable().insert({
                        'contact_email': _model.cityTextController2.text,
                        'contact_name': _model.yourNameTextController.text,
                        'currency': PostOrderCall.oCurrancy(
                          (_model.apiResultrci?.jsonBody ?? ''),
                        ),
                        'payment_status': 'Pending',
                        'product_ids': widget.cart?.map((e) => e.id).toList(),
                        'checkout_url': PostOrderCall.oCheckout(
                          (_model.apiResultrci?.jsonBody ?? ''),
                        ),
                        't_order_id': PostOrderCall.oID(
                          (_model.apiResultrci?.jsonBody ?? ''),
                        ),
                        'order_total': PostOrderCall.oTotal(
                          (_model.apiResultrci?.jsonBody ?? ''),
                        ),
                      });
